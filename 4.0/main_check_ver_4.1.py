...
#2024년 5월 현재시간 데이터 시간까지 출력, 제로값 갯수 추가
...
def get_data(area, page, start, end):
    ...
    try:
        ...
        response.raise_for_status()  # 잘못된 요청(4XX 또는 5XX)에 대해 HTTPError 발생

        # 요청이 성공적이면 HTML 내용 처리
        html_content = StringIO(response.text)
        return pd.read_html(html_content)[0]  # 첫 번째 테이블이 관련 데이터라고 가정
    ...
    return pd.DataFrame()  # 문제가 발생했을 경우 빈 DataFrame 반환
...
def get_judge_data(data_concat,bad_area):
    ...
    if data_index_cnt < values_cnt -10: # -1 홈페이지에서 10분 늦게올라와서 -1을 해야 정상값
        # print(bad_area)
        # dust_state = "미세먼지데이터를 확인해주세요"
        # print(dust_state)
        # print("--------------today_data_cnt :", data_index_cnt,"--------------now_data_cnt", values_cnt-2)
        if data_index_cnt == 0:
            # If no valid data entries, add the area to the final_count_data_zero list
            final_count_data_zero.append(bad_area)
        else:
            # If there are fewer entries than expected, add the area and the count to the final_count_data_name list
            if data_index_cnt < values_cnt -10:
                time_data = time_data[~time_data.applymap(lambda x: x == '조회된 이력이 없습니다.').any(axis=1)]
                time_data = time_data.values
                if len(time_data) > 1:
                    recent_time = time_data[-1]
                    # recent_time이 numpy.ndarray 인스턴스인지 확인
                    if isinstance(recent_time, np.ndarray):
                        if isinstance(recent_time, np.ndarray) and recent_time.size > 0:
                            datetime_str = recent_time[0]  # Split on space and take the first element
                            final_count_data_name.append((bad_area, data_index_cnt, datetime_str))
                # recent_time = time_data[-2]

                # final_count_data_name.append((bad_area, data_index_cnt, recent_time))
    ...
    if condition_zero['col1'] |condition_zero['col2'] | condition_zero['col3']|condition_zero['col4'] |condition_zero['col5'] | condition_zero['col6']:                          #데이터값에 음수가있으면 출력한다.
        ...
        # Count the number of zeros in each relevant column
        zero_counts = zero_values_mask[['col1', 'col2', 'col3', 'col4', 'col5', 'col6']].sum().sum()

        # Check if there are any zeros in the specified columns and append bad_area and zero count if true
        if zero_values_mask[['col1', 'col2', 'col3', 'col4', 'col5', 'col6']].any().any():
            final_zero_state.append((bad_area, zero_counts))
            # print(f"{bad_area}: Zero values count - {zero_counts}")
    ...
    if condition_under['pm10'] |condition_under['pm2.5'] | condition_under['pm1.0']:                          #데이터값에 음수가있으면 출력한다.
        if final_result_data_tail_6['pm10'] | final_result_data_tail_6['pm2.5'] | final_result_data_tail_6['pm1.0']:
            if final_plus_result_data_tail_6['pm10'] | final_plus_result_data_tail_6['pm2.5'] | final_plus_result_data_tail_6['pm1.0']:
                ...
                final_under_date.append((bad_area, time_add_result_data.tail(6)))
...
total_area = [ 
            ...
            "0371","0372","0373",   #선릉_5m,        선릉_12m,             선릉_19m
            "0381","0382","0383",   #강화_월곶,      강화_평화,             강화_석모도
            "0391","0392","0393",   #철원_고석정,     철원_평화,            철원_고대산
            "0401","0402","0403",   #파주_쇠재,      파주_월롱,            파주_체육
            "0411","0412","0413",   #괴산_차단숲,    증평_저감숲,            괴산_국유림
            "0421","0422","0423",   #포항_저감숲,    포항_차단숲,            포항_농경지
            "0431","0432","0433",   #영주_저감숲,    영주_농경지,            영주_치유원
            "0441","0442","0443",   #미시령_고개,    미시령_활엽,            미시령_침엽
            "0451","0452","0453",   #창경궁_지상,    창경궁_10m,            창경궁_19m
...
print("과거 자료 없는 관측지점 : ", final_count_data_zero)
