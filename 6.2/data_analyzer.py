...
class DataAnalyzer:
    def __init__(self, list_views, missing_threshold=10, negative_tail=6, zero_threshold=1):
    ...
    def process_data(self, data):
        ...
        # 컬럼 수 검증: 반드시 6개여야 함 (산림 3개 + 산업 3개)
        actual_cols = normal_data.shape[1]
        if actual_cols != 6:
            raise ValueError(
                f"미세먼지 컬럼 수 불일치 — 예상: 6개, 실제: {actual_cols}개. "
                f"API 응답 구조가 변경되었을 수 있습니다. 컬럼 목록: {list(normal_data.columns)}"
            )
    ...
    def handle_counts_and_conditions(self, bad_area, processed_data):
        ...
        if data_index_cnt < self.values_cnt - self.missing_threshold:
            if data_index_cnt == 0:
                self.final_count_data_zero.append(bad_area)
            else:
        ...
        if condition_zero.any():
            ...
            if zero_counts >= self.zero_threshold:
        ...
        if condition_under.any():
            result_data_tail_n = result_data.tail(self.negative_tail)
            final_result_data_tail_6 = (result_data_tail_n < 0).any()
            final_plus_result_data_tail_6 = (result_data_tail_n > 0).any()
            if final_result_data_tail_6.any() and not final_plus_result_data_tail_6.any():
                min_value_row = time_add_result_data.tail(self.negative_tail).min()
                max_value_row = time_add_result_data.tail(self.negative_tail).max()
